...
- Una volta processati i campioni di una sequenza, costruire un dizionario con le seguenti chiavi:
    ...
    * `gt`: Tensor di forma (N, 3), con colonne (distance, sin(angle), cos(angle))
...
def process_sequence(seq_name):
    ...
    for _, row in gt_df.iterrows():
        ...
        # Estrae i target di distanza e angolo
        dist = row['dist']
        angle_deg = row['angle']
        ...
        # (N, 3) con colonne (distance, sin(angle), cos(angle))
        gt_tensor = torch.tensor([dist, sin_angle, cos_angle], dtype=torch.float32)
    ...
    if len(spectrogram_list) > 0:
        ...
        final_gt = torch.stack(gt_list)             # Shape: (N, 3)
